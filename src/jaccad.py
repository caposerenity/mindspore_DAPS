...
import faiss
import mindspore
from faiss_utils import search_index, search_raw_array, \
...
def compute_jaccard_distance(target_features, k1=20, k2=6, print_flag=True, search_option=0, use_float16=False):
    ...
    if (search_option==0):
        ...
        _, initial_rank = search_raw_array(res, target_features, target_features, k1)
        initial_rank = initial_rank.cpu().numpy()
    elif (search_option==1):
        res = faiss.StandardGpuResources()
        index = faiss.GpuIndexFlatL2(res, target_features.size(-1))
        index.add(target_features.cpu().numpy())
        _, initial_rank = search_index(index, target_features, k1)
    ...
    for i in range(N):
        ...
        dist = 2-2*mindspore.ops.mm(target_features[i].unsqueeze(0).contiguous(), target_features[k_reciprocal_expansion_index].t())
        if use_float16:
